...
import { VisualChatInterface } from '../components/chat/VisualChatInterface';
import { ParticleLogin } from '../components/auth/ParticleLogin';
import { DevLogin } from '../components/auth/DevLogin';
import { useParticleAuth } from '../hooks/useParticleAuth';

// 合約地址 (本地 Hardhat 網絡部署地址)
const PERSONA_NFT_ADDRESS_LOCAL = '0x5FbDB2315678afecb367f032d93F642f64180aa3' as `0x${string}`;

export default function Home() {
  const { address, isConnected } = useAccount();
  const { isLoggedIn, currentUser, userPersonas, activePersona } = useParticleAuth();
  ...
  const mintCorePersona = () => {
    ...
    writeContract({
      ...
      args: [
        ...
        [BigInt(1), BigInt(2), BigInt(3), BigInt(4), BigInt(5), BigInt(6), BigInt(7), BigInt(8), BigInt(9), BigInt(10)] // traits array
      ],
  ...
  return (
    <div className="flex flex-col min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100">
             {/* 連接按鈕 */}
             <header className="fixed top-4 right-4 z-50 flex gap-3 items-center">
               {/* Dev 登入（優先顯示以便無錢包也可測試） */}
               <DevLogin />
               {/* 原本的錢包登入仍保留 */}
               {isLoggedIn ? <ParticleLogin /> : <ConnectWallet />}
             </header>
        ...
        <div className="mb-8 text-center">
               ...
               <div className="mb-8 p-6 bg-white rounded-lg shadow-lg w-full max-w-md">
                 <p className="text-lg text-center">
                   {isLoggedIn ? (
                     <span className="text-green-600">
                       🟢 Particle 已連接: {currentUser?.walletAddress?.slice(0, 6)}...{currentUser?.walletAddress?.slice(-4)}
                     </span>
                   ) : (
                     <span className="text-red-600">
                       🔴 請連接 Particle 錢包以開始育成 AI 人格
                     </span>
                   )}
                 </p>
                 ...
                 {isLoggedIn && (
                   <div className="mt-4 text-sm text-gray-600 text-center space-y-1">
                     <p>您的 AI 人格數量: <span className="font-semibold">{userPersonas.length}</span></p>
                     <p>激活人格: <span className="font-semibold">{activePersona?.name || '無'}</span></p>
          ...
          <div className="mt-8 p-8 bg-white rounded-xl shadow-lg max-w-2xl">
            ...
            <p className="text-gray-600 mb-6 text-center">
              使用 Particle Network 連接，鑄造第一個 AI 人格 NFT，體驗 Web3 + AI 的無限可能
            </p>
            <div className="text-sm text-gray-500 text-center space-y-2">
              <p>• 支援 Particle Network 多種登入方式</p>
              <p>• 基於 Base Sepolia 測試網</p>
              <p>• 安全、快速、低費用</p>
              <p>• 與 AI 人格進行深度對話</p>
              <p>• 完整的 Chatkit 對話管理系統</p>
            </div>
          </div>
        )}
      </main>
